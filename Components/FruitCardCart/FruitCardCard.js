...
export default function FruitCardCart({fruit}) {
  ...
  const handelUpdateQuantity = qty => {
    if (qty == 0) {
      Alert.alert(
        'Remove From Cart',
        `Remove ${fruit.name} from cart?`, // <- this part is optional, you can pass an empty string
        [
          {text: 'Cancle', onPress: () => {}},
    ...
    } else dispatch(updateQty({id: fruit.id, qty}));
  };
  return (
    <View className="flex-row justify-between items-center space-x-5 mb-4">
      <View className="ml-5">
        ...
        <View
          ...
          className={` rounded-3xl flex justify-end items-center -mt-2`}></View>
      </View>
      ...
      <View className="flex-row items-center space-x-2">
        <TouchableOpacity
          ...
          onPress={() => handelUpdateQuantity(fruit.qty + 1)}>
