  ...
  return (
    <SafeAreaView className="flex-1 bg-orange-50">
      ...
      <View className='flex-row justify-between items-center mx-5 py-2'>
        ...
        <TouchableOpacity className='bg-orange-100 p-2 rounded-xl' onPress={()=>navigation.navigate("Cart")}>
          <ShoppingCartIcon size='35' color='orange' />
        </TouchableOpacity>
      </View>
      <View className='mt-4'>
        <Text className='text-2xl tracking-widest font-medium ml-5'>Seasonal</Text>
        <Text className='text-3xl tracking-widest font-semibold ml-5'>Fruits And Vegetables</Text>
        {/* category */}
        <ScrollView className="mt-8 px-5" horizontal showsHorizontalScrollIndicator={false}>
          {categories.map((category, index) => {
            ...
            return <TouchableOpacity onPress={() => { setActiveCategory(category) }} key={index}>
              <Text className={`mr-8 relative ${textClass}`}>{category}</Text>
              {isActive && <Text className="ml-2 color-orange-400 font-extrabold">__ _</Text>}
            </TouchableOpacity>
          })}
        </ScrollView>

        {/* fruits corousel */}
        <ScrollView className="mt-8" horizontal showsHorizontalScrollIndicator={false}>
